...
namespace TNPA.Droid
{
    ...
    public class DanhSachPhanAnhActivity : AppCompatActivity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            // Lấy object lĩnh vực được truyền qua từ màn hình trước
            var json = Intent.GetStringExtra("dsLinhVuc");
            var linhvuc = JsonConvert.DeserializeObject<DmLinhVuc>(json);
            _listLinhVuc = new List<DmLinhVuc>();
            _listLinhVuc.Add(linhvuc);
            // Set title
        ...
        private void OnListItemClick(object sender, AdapterView.ItemClickEventArgs e)
        {
            ...
            {
                Intent intent = new Intent(this, typeof(ChiTietPhanAnhActivity));

                var json = JsonConvert.SerializeObject(_items.Select(p => p.PhanAnhID));
                intent.PutExtra("dsPhanAnh", json);
                intent.PutExtra("Position", e.Position);
                //intent.PutExtra("totalItem", _TotalItem);
                intent.PutExtra("totalItem", _items.Count);
        ...
        public async void getDanhSachPhanAnh()
        {
            ...
            var result = await _service.GetListPhanAnhByLinhVuc(_listLinhVuc, searchKey, _PageIndex, Config.PageSize);
            if (result != null)
            {
            ...
            else
            {
                ...
                alert.SetTitle(Resources.GetString(Resource.String.loi));
                alert.SetMessage(Resources.GetString(Resource.String.loidata));
                alert.SetNegativeButton(Resources.GetString(Resource.String.dongy), (senderAlert, args) =>
                {
                    Dispose();
                });
